...
function containerTypeFromTrace(trace) {
  ...
  if (tag === "form" || role === "form") {
    if (className.includes("composer") || className.includes("prompt") || className.includes("inputbar") || className.includes("chat")) return "composer-form";
    return "form-section";
  }
  if (className.includes("composer") || className.includes("prompt") || className.includes("inputbar")) return "composer-form";
  ...
  if (tag === "table" || role === "table" || role === "grid" || className.includes("table")) return "table";
  if (tag === "ul" || tag === "ol" || role === "list" || className.includes("list")) return "list-group";
...
function semanticNameWithSuffix(name, suffix, component) {
  ...
  if (component.category === "tag" && /\b(tag|badge|chip|pill)\b/.test(lower)) return cleaned;
  if (component.category === "form-field" && /\b(field|input)\b/.test(lower)) return cleaned;
...
function semanticSuffix(component) {
  ...
  if (category === "breadcrumb") return "Breadcrumb";
  if (category === "form-field") return "Field";
...
function spriteId(src) {
  ...
  window.ReverseDesignSystemNormalizer = {
    normalize
  };
})();
